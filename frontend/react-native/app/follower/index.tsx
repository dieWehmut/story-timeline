    ...
    return (
      <Screen footer={<FooterBar />} footerHeight={86}>
        ...
        <View style={styles.loginBlock}>
          <Text style={[styles.loginHint, { color: colors.textSoft }]}>请先登录查看粉丝列表</Text>
          <Pressable
      ...
      <FlatList
        ListHeaderComponent={
          <>
            <TopBar
              title="粉丝"
              subtitle={`${follows.followers.length}`}
              showBack
              rightAction={
                <View style={styles.actions}>
                  <HomeButton />
        ...
        renderItem={({ item }) => (
          <UserRow
            ...
            followed={follows.isFollowing(item.login)}
            ...
            onToggle={() =>
              follows.isFollowing(item.login)
                ? follows.unfollow(item.login)
                : follows.follow(item.login, item.avatarUrl)
            }
          />
        )}
        ListEmptyComponent={
          <Text style={[styles.emptyText, { color: colors.textSoft }]}>暂无粉丝。</Text>
        }
      />
    </Screen>
  );
